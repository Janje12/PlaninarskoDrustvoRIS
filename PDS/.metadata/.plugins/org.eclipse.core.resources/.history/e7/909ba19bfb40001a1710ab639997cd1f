		...
		<table>
			...
			<td rowspan="9" style="width:100%">
				...
				<table>
					...
					<tr><th>ID Korisnika</th><th>Ime</th><th>Prezime</th><th>Username</th><th>Sifra</th><th>Datum Rodjenja</th><th>Uloga</th>
					<th>Prihvati u planinara</th></tr>
					<c:forEach items="${korisnici}" var="i">
						<tr><td>${i.idKorisnik}</td><td>${i.ime}</td><td>${i.prezime}</td><td>${i.username}</td><td>${i.sifra}</td>
						<td>${i.datumRodjenja}</td><td>${i.uloga.naziv}</td>
						<td><a href="/pdsWEB/Korisnik/prihvati?idKorisnika=${i.idKorisnik}">Prihvati</a></td></tr>
					</c:forEach>
				</table>
				</c:if>
			</td>
